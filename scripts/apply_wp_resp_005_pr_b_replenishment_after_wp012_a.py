...
def main() -> None:
    ...
    assert wp13["selectable"] is True and wp13["ready_state"] == "active"
    ...
    assert (ROOT / "validation/e2e/run_prompt_5_routing_envelope_check.py").exists()
    assert (ROOT / "docs/48_PROMPT_5_PROJECT_GATE_ROUTING_BOUNDARY.md").exists()

    wp13["current_state"] = "PRs #169, #172, and #173 completed the Prompt 5 routing contract/schema, fixture/validator/CI, and documentation/STATUS slices; the package is consumed and no longer selectable-ready or active."
    wp13["measurable_current_state"] = {
        "statement": wp13["current_state"],
        "measurement": "Measured by merged PRs #169, #172, and #173 plus the presence of the Prompt 5 routing schema, validator, CI wiring, documentation, command-index, and STATUS parity artifacts.",
        "measurable": True,
    }
    ...
    active_ids = [package_id for package_id, package in catalog["work_packages"].items() if package.get("ready_state") == "active"]
    assert ready_ids == ["WP-RESP-010", "WP-RESP-011", "WP-RESP-014", "WP-RESP-015"], ready_ids
    assert active_ids == ["WP-RESP-012"], active_ids

    expected_global_boundary_keys = {
    ...
    assert set(catalog["global_boundaries"]) == expected_global_boundary_keys
